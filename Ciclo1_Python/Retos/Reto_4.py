# -*- coding: utf-8 -*-
"""
Created on Sat Oct 17 19:00:11 2020
...
def promedio_facultades(info: dict, contando_externos : bool = True ) -> tuple:
    """
    Calcula el promedio ponderado por facultad de un muestreo aleatorio

    Parameters
    ----------
    info : dict
        Clave: str= {año ingreso}{periodo ingreso}{5 numeros adicionales}.
        valor: dict= nombres: str = {primer nombre} {segundo nombre}
                     apellidos: str = {segundo apellido}, {primer apellido}
                     documento: int 
                     Tipo: str = ('CC'/'TI')
                     programa: str = Codigo del programa
                     materias: list = dict = facultad: str = Facultad a la que pertenece
                                             Codigo: Str = {codigo programa}-{4 numeros}
                                             Nota: Float = Nota obtenida
                                             creditos: Int = Numero de creditos de la materia
                                             retirada: str = ('Si'/'No')
    contando_externos : bool, optional
        False: no se consideran las notas de las materias que no coinciden en el programa del estudiante
                o que esten en periodo 'curso de verano'= 05. 
        The default is True.

    Returns
    -------
    tuple
        [0] : dict = clave: str = nombres de las facultades ordenadas alfabeticamente.
                     valor: str = promedio ponderado de las materias
        [1] : list =  correos institucionales: Si Dos nombres = {1° letra 1° nombre}{1° letra 2° nombre}
                                                            .{1° apellido}{dos ultimos numeros del documento}
                                               SiNo Un nombre = {1° letra del 1° nombre}{1° letra del 1° apellido}
                                                           .{2° apellido}{dos últimos números del documento}

    Excepts                                                           
    -------
    calcular promedio, si hay un error al calcular el promedio.
    ...
    for key , value in info.items():
        ...
        #creamos el correo
        Nombres = value["nombres"].split()
        Apellidos = value["apellidos"].split()
        correo = Nombres[0][0]
        try:
            correo += Nombres[1][0]
            correo += '.' + Apellidos[1]
        except: 
            correo += Apellidos[1][0]
            correo += '.'+ Apellidos[0][:-1]
        
        correo += str(value["documento"] % 100).zfill(2)
        correo = normalizar(correo).lower()
       
         #se compara la condicion si el estudiante esta en periodo curso de verano
        
        for materia in value["materias"]:
            if materia["creditos"] == 0 or materia["retirada"] == "Si" or (contando_externos == False and (value["programa"] != materia["codigo"].split('-')[0] or perin == 5)):
                continue
            ...
            try:
                if materia["facultad"] not in Facultades:
                    Facultades[materia["facultad"]] = materia["nota"] * materia["creditos"]
                    Creditos[materia["facultad"]] = materia["creditos"]
                else:
                    Facultades[materia["facultad"]] += materia["nota"] * materia["creditos"]
                    Creditos[materia["facultad"]] += materia["creditos"]
            except:
    ...
    try:
        for Facultad in Facultades:
            Facultades[Facultad] = round(Facultades[Facultad] / Creditos[Facultad],2)
            
    except:
        return "Error numérico."
    #ordenar los valores
    Facultades = {k: v for k, v in sorted(Facultades.items())}
    Correos.sort()
    return tuple((Facultades, Correos))
